"""Run the existing full benchmark while capturing selected pricing inputs.
...
def main() -> int:
    """Capture rounds 1, 6, and 11 during one existing benchmark child run."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--source-root", type=Path, required=True)
    parser.add_argument("--benchmark", type=Path, required=True)
    parser.add_argument("--out", type=Path, required=True)
    parser.add_argument("--rounds", type=int, nargs="+", default=[1, 6, 11])
    parser.add_argument("--expected-production-flights", type=int, default=1537)
    ...
    def observed(
        ...
        **kwargs,
        ...
        all_call_number += 1
        # benchmark_colgen_lns performs a two-flight compilation warmup before
        # generating the requested 1,537-flight production batch.
    ...
    manifest.update(
        status="complete" if return_code == 0 and not missing else "failed",
