...
def run_batch(
    ...
    *,
    ...
    """
    # Round-trip itineraries are not representable here. Colgen prices COLUMNS — one origin→dest path
    # per flight — so a `return_to_origin` request would be solved as its outbound leg alone and the
    # return would vanish silently, which is indistinguishable in the output from a one-way delivery.
    # Refuse instead: a whole-schedule planner that quietly drops half of every trip is worse than one
    # that does not run.
    ...
    if bad:
        raise NotImplementedError(
            ...
            "would be dropped without a trace. Use a per-flight planner, or generate the demand "
            "with return_flights=False.")
