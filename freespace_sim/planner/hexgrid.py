...
def _covered_boundary(center, term, cfg: SimConfig) -> tuple[set, set]:
    """Flood-fill a hub's column: ``covered`` hexes (centre dist < ``exit_radius``) and the ``boundary``
    ring just outside them. Shared by :func:`terminal_lanes` (boundary = the exit lanes) and
    :func:`terminal_cells` (covered ∪ boundary = the full reserved terminal airspace)."""
    cx, cy = float(center[0]), float(center[1])
    R = circumradius(cfg)
    er = exit_radius(as_terminal(term), cfg)
    covered: set = set()
...
def terminal_lanes(center, term, cfg: SimConfig) -> list[Lane]:
    ...
    if cached is not None:
        return cached
    R = circumradius(cfg)
    _, boundary = _covered_boundary(center, term, cfg)
