...
SCENARIOS: dict[str, ScenarioSpec] = {
    ...
    "dallas_hub_2uss": ScenarioSpec(
        ...
        # No pads_per_hub: HubVoronoiDemand has no pad model, so the field would be silently dropped
        # and misread as a capacity limit this scenario lacks. Use dallas_hub_2uss_large for pads.
    ...
    # the full metro world: 60×45 km, 20 Walmarts + 240 strip malls, λ=34.5k deliveries (~2× with
    # returns), demand split 1:2 Walmart:strip-mall (uss_share). Pads AND boundary-hex exit lanes
    # are provisioned ABOVE peak per-hub demand so neither constrains takeoffs — leaving air
    # congestion as the only delay source. A pad dwell holds ~55 s (hover+climb), an exit lane
    # only ~12 s (transit), so lanes clear ~4.6× faster: pads bind first and lanes keep headroom.
