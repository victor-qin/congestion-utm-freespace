...
def _payload(result: SimResult) -> dict:
    """Flatten every accepted intent into a complete, JSON-serialisable replay scene."""
    cfg = result.config
    hues = result_uss_hues(result)
    usses = sorted(hues)
    uss_ix = {u: i for i, u in enumerate(usses)}
    # The replay is the realized operation, not the configured planning envelope: start when the first
    # accepted flight reserves airspace and stop only after the final landing reservation clears.
...
let DATA=null, LEVELS=[], DURATION=0, IQ=1, IQT=1;        // IQ/IQT: quantised unit → metres / seconds
...
async function boot(){{
  ...
  DURATION = Math.max(0, DATA.simulation_duration_s);
...
// cannot hold sub-unit advances and speeds < 1 would round away. At 1x the complete realized run plays
// in ~10s (60fps · duration/600 per frame); speed scales that per-frame step.
...
def write_html(result: SimResult, out) -> str:
    """Render the complete realized run to a standalone HTML scrubber; return the output path."""
    ...
    html = _HTML.format(
        start=json.dumps(payload["simulation_start_s"]),
