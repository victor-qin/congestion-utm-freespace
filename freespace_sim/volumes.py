...
def corridor_segment_volume(
    p0: Vec, t0: float, p1: Vec, t1: float, cfg: SimConfig
) -> Volume4D:
    ...
    **This is the contract between the planners and the ledger.** A planner (RRT* per edge, or the
    ...
    return Volume4D(spec, t0 - cfg.time_buffer_s, t1 + cfg.time_buffer_s)
...
def hover_reservation(center: Vec, t0: float, cfg: SimConfig) -> Volume4D:
    ...
    descent; active for ``hover_time_s + climb_time_s`` from ``t0``.
    """
    ...
    return Volume4D(spec, t0, t0 + cfg.hover_time_s + cfg.climb_time_s)
