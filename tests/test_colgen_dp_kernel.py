...
def test_variant_chunking_is_exact(monkeypatch):
    """Chunked search must return the same column as the single sweep.

    Chunking applies dominance only within a batch, so labels the monolithic sweep would
    have dominated across departure variants survive -- it explores a SUPERSET.  That is
    why it stays exact, and why it costs more work.  The residual is accumulated as a max
    over batches so the optimality proof still covers what every batch left unexplored;
    if that were dropped the answer would still look right while `proved` silently lied.
    """
    ...
    for trial in range(6):
        cfg = _cfg(max_ground_delay_s=120.0)
        graph, params = _graph(
            cfg, dest=(int(rng.integers(3, 6)), int(rng.integers(-2, 2))),
            slack=3, flight_id=trial + 1,
        )
        cells = sorted(graph.corridor_cells)
        view = DualView(_random_duals(rng, cells, graph.base_step, 25), cfg)

        with monkeypatch.context() as patch:
            patch.setattr(pricing, "_VARIANT_CHUNK", 0)
            base_rc, base = price_flight(graph, view, 1.0, cfg, params,
                                         require_improving=False)
        with monkeypatch.context() as patch:
            patch.setattr(pricing, "_VARIANT_CHUNK", 2)   # force several batches
            got_rc, got = price_flight(graph, view, 1.0, cfg, params,
                                       require_improving=False)
        ...
        compared += 1
        assert got is not None
        assert got_rc == pytest.approx(base_rc, abs=1e-8)
        assert got.cell_path == base.cell_path
        assert got.departure_step == base.departure_step
    assert compared >= 3, f"only {compared} graphs produced a column; sweep too weak"
