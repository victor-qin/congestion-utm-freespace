...
def test_colgen_test_fast_real_batch_smoke(monkeypatch):
    spec = with_overrides(
    ...
    monkeypatch.setattr(
        "freespace_sim.sim.get_planner",
        lambda _name: ColumnGenerationPlanner(params),
    )
...
def full_colgen_test_results():
    ...
    with patch(
        "freespace_sim.sim.get_planner",
        return_value=ColumnGenerationPlanner(
            ColGenParams(time_limit_s=solver_time_limit_s)
        ),
    ):
        colgen = run(cfg, requests=requests, demand=demand, progress=False)
...
def test_colgen_small_hub_seed_sweep_has_no_filing_denials(seed, monkeypatch):
    spec = with_overrides(
        get_scenario("colgen_test"),
        planner="colgen",
        seed=seed,
