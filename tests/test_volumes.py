...
def test_horizontal_corridor_box_geometry_is_unchanged():
    ...
    # the lane link box drops ONE end's extension (it must touch the column edge, not re-enter it)
    lo, hi = corridor_segment_volume(vec(0, 0, z), 0.0, vec(seg, 0, z), CFG.dt_s, CFG,
                                     extend_start=False).shape.aabb()
    assert np.isclose(lo[0], 0.0) and np.isclose(hi[0], seg + ext)
    lo, hi = corridor_segment_volume(vec(0, 0, z), 0.0, vec(seg, 0, z), CFG.dt_s, CFG,
                                     extend_end=False).shape.aabb()
    assert np.isclose(lo[0], -ext) and np.isclose(hi[0], seg)
